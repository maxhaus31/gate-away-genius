...
def _call_gemini(prompt: str, model: str = PRIMARY_MODEL, call_name: str = "gemini", thinking_budget: int = None) -> str:
...
def _call_gemini_with_fallback(prompt: str, call_name: str = "gemini", thinking_budget: int = None) -> str:
    """Try primary model, fall back to lite on failure. Caches results to avoid repeat hits."""
    cached = _cache_get(prompt)
    if cached is not None:
        return cached
    try:
        result = _call_gemini(prompt, PRIMARY_MODEL, call_name=call_name, thinking_budget=thinking_budget)
...
Return ONLY a valid JSON array, nothing else."""
        ...
        try:
            text = _call_gemini_with_fallback(prompt_text, call_name="generate_persona_places", thinking_budget=1000)
            if "```json" in text:
