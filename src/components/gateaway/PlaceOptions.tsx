...
export const PlaceOptions = ({ places, selectedIds, onPlaceSelect, onPlaceDeselect }: Props) => {
  ...
  const maxPlaces = 3;
  const remainingSlots = maxPlaces - selectedIds.size;
  const isMaxed = selectedIds.size >= maxPlaces;
    ...
    <div className="rounded-2xl border border-border bg-card p-6 sm:p-8">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-sm font-medium text-muted-foreground">
          Selected: {selectedIds.size}/{maxPlaces}
        </div>
      </div>
      <p className="text-sm text-muted-foreground mb-6">
        Click to add up to {maxPlaces} places for your route (45 min per place)
      </p>
      
      <div className="grid gap-3">
        {places.map((place) => {
          const isAdded = selectedIds.has(place.place_id);
          const canAdd = !isAdded && !isMaxed;
          
          return (
          <button
            key={place.place_id}
            onClick={() => canAdd && onPlaceSelect(place)}
            ...
            {place.photo_url && (
              <div className="relative h-40 w-full overflow-hidden bg-muted">
                <img 
                  src={place.photo_url} 
                  alt={place.name}
                  className={`h-full w-full object-cover ${canAdd ? "group-hover:scale-105" : ""} transition-transform`}
                />
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <h3 className={`font-semibold ${canAdd ? "group-hover:text-primary transition-colors" : isAdded ? "text-green-700 dark:text-green-400" : "text-foreground"}`}>
                      {place.name}
                    </h3>
                    {isAdded && (
